...
def calculate_threshold_contrast_advanced(
    ...
    # 3. Perform numerical integrations
    max_freq = np.max(mtf_data[:, 0])
    numerator_integral, _ = quad(numerator_integrand, 0, nyquist_freq, limit=100)
    denominator_integral, _ = quad(denominator_integrand, 0, max_freq, limit=100)
...
def display_threshold_contrast_section():
    ...
    st.metric("Nyquist Frequency", f"{nyquist_freq:.2f} cy/mm")
    if mtf_ready:
        st.metric("Max MTF Frequency", f"{np.max(st.session_state['mtf_data'][:, 0]):.2f} cy/mm")
    ...
    if st.button("Calculate Threshold Contrast", disabled=not (mtf_ready and nnps_ready)):
        with st.spinner("Calculating..."):
            ...
            ct = calculate_threshold_contrast_advanced(
                object_radius=object_radius, magnification=magnification,
                mtf_data=st.session_state['mtf_data'], nnps_data=st.session_state['nnps_data'],
